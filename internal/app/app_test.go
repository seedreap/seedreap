...
func TestRegistry(t *testing.T) {
	...
	t.Run("Register", func(t *testing.T) {
		...
		sonarr := app.NewPassthrough("sonarr", "tv-sonarr", "/downloads/tv")
		r.Register("sonarr", sonarr)

		// Verify app is retrievable by name
		got, ok := r.Get("sonarr")
		require.True(t, ok)
		assert.Equal(t, sonarr, got)

		// Verify it's in All()
		all := r.All()
		assert.Len(t, all, 1)
		assert.Equal(t, sonarr, all["sonarr"])

		// Verify it's indexed by category
		byCategory := r.GetByCategory("tv-sonarr")
		assert.Len(t, byCategory, 1)
		assert.Equal(t, sonarr, byCategory[0])
	})

	t.Run("RegisterMultipleApps", func(t *testing.T) {
		r := app.NewRegistry()

		sonarr := app.NewPassthrough("sonarr", "tv-sonarr", "/downloads/tv")
		radarr := app.NewPassthrough("radarr", "movies", "/downloads/movies")
		misc := app.NewPassthrough("misc", "misc", "/downloads/misc")

		r.Register("sonarr", sonarr)
		r.Register("radarr", radarr)
		r.Register("misc", misc)

		assert.Len(t, r.All(), 3)
		assert.Len(t, r.Categories(), 3)
	})

	t.Run("RegisterMultipleAppsForSameCategory", func(t *testing.T) {
		r := app.NewRegistry()

		sonarr := app.NewPassthrough("sonarr", "tv", "/downloads/tv")
		sonarr4k := app.NewPassthrough("sonarr-4k", "tv", "/downloads/tv-4k")

		r.Register("sonarr", sonarr)
		r.Register("sonarr-4k", sonarr4k)

		// Both should be returned for the same category
		byCategory := r.GetByCategory("tv")
	...
	t.Run("GetByCategory_Empty", func(t *testing.T) {
		...
		byCategory := r.GetByCategory("nonexistent")
		assert.Empty(t, byCategory)
	})

	t.Run("RegisterForDownloader", func(t *testing.T) {
		r := app.NewRegistry()

		sonarr := app.NewPassthrough("sonarr", "tv", "/downloads/tv")
		r.RegisterForDownloader("seedbox", sonarr)

		byDownloader := r.GetByDownloader("seedbox")
		assert.Len(t, byDownloader, 1)
		assert.Equal(t, sonarr, byDownloader[0])
	})

	t.Run("GetByDownloader_Empty", func(t *testing.T) {
		r := app.NewRegistry()

		byDownloader := r.GetByDownloader("nonexistent")
		assert.Empty(t, byDownloader)
	})

	t.Run("Categories", func(t *testing.T) {
		r := app.NewRegistry()

		r.Register("sonarr", app.NewPassthrough("sonarr", "tv", "/downloads/tv"))
		r.Register("radarr", app.NewPassthrough("radarr", "movies", "/downloads/movies"))
