...
func (_q *SyncJobQuery) QueryDownloadJob() *DownloadJobQuery {
...
func (_q *SyncJobQuery) QueryFiles() *SyncFileQuery {
...
func (_q *SyncJobQuery) FirstID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (_q *SyncJobQuery) FirstIDX(ctx context.Context) ulid.ULID {
...
func (_q *SyncJobQuery) OnlyID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (_q *SyncJobQuery) OnlyIDX(ctx context.Context) ulid.ULID {
...
func (_q *SyncJobQuery) IDs(ctx context.Context) (ids []ulid.ULID, err error) {
...
func (_q *SyncJobQuery) IDsX(ctx context.Context) []ulid.ULID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *SyncJobQuery) WithDownloadJob(opts ...func(*DownloadJobQuery)) *SyncJobQuery {
	query := (&DownloadJobClient{config: _q.config}).Query()
...
func (_q *SyncJobQuery) WithFiles(opts ...func(*SyncFileQuery)) *SyncJobQuery {
	query := (&SyncFileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(syncjob.FieldCreatedAt).
...
func (_q *SyncJobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SyncJob, error) {
	...
	if query := _q.withDownloadJob; query != nil {
		if err := _q.loadDownloadJob(ctx, query, nodes, nil,
			func(n *SyncJob, e *DownloadJob) { n.Edges.DownloadJob = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFiles; query != nil {
		if err := _q.loadFiles(ctx, query, nodes,
			func(n *SyncJob) { n.Edges.Files = []*SyncFile{} },
			func(n *SyncJob, e *SyncFile) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SyncJobQuery) loadDownloadJob(ctx context.Context, query *DownloadJobQuery, nodes []*SyncJob, init func(*SyncJob), assign func(*SyncJob, *DownloadJob)) error {
	ids := make([]ulid.ULID, 0, len(nodes))
	nodeids := make(map[ulid.ULID][]*SyncJob)
	for i := range nodes {
...
func (_q *SyncJobQuery) loadFiles(ctx context.Context, query *SyncFileQuery, nodes []*SyncJob, init func(*SyncJob), assign func(*SyncJob, *SyncFile)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(syncfile.FieldSyncJobID)
	}
	query.Where(predicate.SyncFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(syncjob.FilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "sync_job_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *SyncJobQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(syncjob.Table, syncjob.Columns, sqlgraph.NewFieldSpec(syncjob.FieldID, field.TypeString))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withDownloadJob != nil {
			_spec.Node.AddColumnOnce(syncjob.FieldDownloadJobID)
		}
	}
